...
namespace NB21_logger
{
    ...
    public partial class NB21Logger : UserControl
    {
        ...
        private readonly bool isInDesignMode = IsRunningInDesignMode();
        ...
        public NB21Logger(bool LaunchedViaStartup)
        {
            ...
            ws_connected = ui_web_urls_connected;
            ws_disconnected = ui_web_urls_disconnected;

            if (isInDesignMode)
            {
                ui_conn_status.Text = "Design mode";
                set_ui_web_urls("Design mode - services inactive");
                return; // skip runtime init while in the designer
            }
            ...
            nb21_ws_server.start(54179);

            var web_addresses1 = new List<string> { $"http://{localIpAddress}:54178/", $"http://localhost:54178/" };
            var web_addresses2 = new List<string> { $"http://localhost:54178/" };

            nb21_web_server = new NB21WebServer(this);
            ui_web_urls_disconnected();
            try
            {
                set_ui_web_urls($"{web_addresses1[1]}b21_task_planner [ip {localIpAddress} ]");
                nb21_web_server.start(web_addresses1);
            }
            catch (HttpListenerException ex) when (ex.ErrorCode == 5)
            {
                set_ui_web_urls(web_addresses2[0] + "b21_task_planner");
                nb21_web_server.start(web_addresses2);
            }

            simdata = new SimDataConn(this);
            igc_file_writer = new IGCFileWriter(simdata);
            simdata.SetWindowHandle(this.Handle);
        }
