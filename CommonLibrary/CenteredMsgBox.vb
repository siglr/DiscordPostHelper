...
Public Class Centered_MessageBox
    ...
    Public Sub New(Optional owner As Form = Nothing)
        If owner Is Nothing Then

            Dim validNames As New List(Of String) From {"Main", "VersionInfoForm", "RecommendedAddOnsForm", "CopyContent", "WizardNextChoice", "WaitingForURLForm", "DPHXUnpackAndLoad", "Settings", "UpdaterForm"}

            For Each f As Form In Application.OpenForms.OfType(Of Form)
                If validNames.Contains(f.Name) Then
                    owner = f
                    Exit For
                End If
            Next
