...
// === Fastest Avg Speed per Glider (All Time, Valid Results) ===
try {
    $success = refreshFastestGliderSpeedsCache($pdo);
    if ($success) {
        echo "✔ Fastest glider speeds written to fastestGliderSpeeds.json\n";
    } else {
        echo "❌ Error (Fastest Glider Speeds): Failed writing fastestGliderSpeeds.json\n";
    }
} catch (Exception $e) {
    echo "❌ Error (Fastest Glider Speeds): " . $e->getMessage() . "\n";
}
